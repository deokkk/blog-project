...
<body>
    ...
    <div class="container-fluid row" id="section">
    	<div class="col-1"></div>
    	<div class="col-7">
	        <h1 class="display-4 paddingTop">게시글 관리</h1>
	        <div>
	        	<div class="alignRight">
	        		<a href="${pageContext.request.contextPath}/AddPosts">글쓰기</a>
	        	</div>
	            <table class="table table-hover marginTop">
		            ...
		            <tbody>
		            	<c:forEach var="posts" items="${postsListAll}">
		            		<tr>
		            			<td>${posts.postsNo}</td>
	        ...
	        <div>
	        	<ul class="pagination"  style="justify-content: center;">
					<c:if test="${page.currentPageGroup>1 }">
						<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/PostsListAll?currentPage=${page.currentPageGroup-page.pagePerGroup}"><i class="fas fa-chevron-left"></i></a></li>
					</c:if>
					...
					<c:forEach begin="${page.currentPageGroup}" end="${page.currentPageGroup+page.pagePerGroup-1}" step="1" varStatus="stats">
						<c:if test="${not doneLoop}">
							...
							<c:if test="${(stats.index)!=page.currentPage}">
								<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/PostsListAll?currentPage=${(stats.index)}">${(stats.index)}</a></li>
							</c:if>
					...
					<c:if test="${page.currentPageGroup<page.lastPageGroup }">
						<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/PostsListAll?currentPage=${page.currentPageGroup+page.pagePerGroup}"><i class="fas fa-chevron-right"></i></a></li>
					</c:if>
				</ul>
	        </div>
	    </div>
